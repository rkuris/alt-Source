...
typedef struct {                                                // Charging Profile Structure                   
   ...
   unsigned long  EXIT_ACPT_DURATION;                           // Stay in Accept mode no longer then duration in mS  (Set = 0 to disable Acceptance phase and move directly to OC or Float mode)
   ...
   int            LIMIT_FLOAT_AMPS;                             // During Float, manage system to keep Amps into Battery at or under this value.  Maybe = 0, set = -1 to disable limit.
   unsigned long  EXIT_FLOAT_DURATION;                          // Alternator will stay in Float mode this long (in mS) before entering Post-Float (no charging) mode.  Set = 0UL disable transition to Post-float mode.
   ...
   unsigned long  EXIT_PF_DURATION;                             // Only stay in Post_float mode (no charging) this amount of time.  Set = 0UL to disable times based Post-float exiting and exit only on Voltage.
   ...
   unsigned long EXIT_EQUAL_DURATION;                           // Regulator will not stay in Equalization any longer then this (in mS).  If set = 0, then Equalization mode will be disabled.
   int           EXIT_EQUAL_AMPS;                               // If Amps fall below this value during Equalization, then exit equalization.  Set = 0 to disable exit by Amps and use only time.
